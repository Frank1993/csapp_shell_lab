...
int parseline(const char *cmdline, char **argv) 
{
    ...
    if (*buf == '\'') {
	buf++;
	delim = strchr(buf, '\''); // strchr() used to fild first index where char exist
    }
    else {
	delim = strchr(buf, ' ');
    }
...
void do_bgfg(char **argv) 
{
    ...
    if(argv[1][0] == '%')
    {
        //TODO: atoi("junk") will return 0, will "0" confuse with process 0 or job 0?
    ...
    if(!is_jid)
        pid_jid = pid2jid(pid_jid);

    struct job_t * current_job =  getjobpid(jobs,pid_jid);
